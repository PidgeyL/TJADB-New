...
_TRANSLATIONS_ = 'translations.json'
_PO_LOCATIONS_ = '.'
_MO_FILE_      = os.path.join('LC_MESSAGES', 'django.mo')
_PO_FILE_      = os.path.join('LC_MESSAGES', 'django.po')
...
def get_po_locations():
    run_path = os.path.dirname(os.path.realpath(__file__))
    return os.path.join(run_path, _PO_LOCATIONS_)
...
def generate():
    ...
    for lang, keys in parsed.items():
        ...
        po_path = os.path.join(get_po_locations(), lang, _PO_FILE_)
